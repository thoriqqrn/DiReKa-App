...
class DiabetesHealthService {
  static final FirebaseFirestore _db = FirebaseFirestore.instance;

  static CollectionReference<Map<String, dynamic>> _recordsRef(String uid) {
    return _db.collection('users').doc(uid).collection('diabetes_health_records');
  }

  static Future<void> addRecord(String uid, DiabetesHealthRecord record) async {
    await _recordsRef(uid).doc(record.id).set(record.toMap());
    await FamilyLinkService.mirrorUserSubcollectionWrite(
      sourceUid: uid,
      subcollection: 'diabetes_health_records',
      docId: record.id,
      data: record.toMap(),
    );
  }

  static Future<void> updateRecord(String uid, DiabetesHealthRecord record) async {
  ...
  static Future<List<DiabetesHealthRecord>> getRecords(
    String uid, {
    DateTime? fromDate,
    int limit = 200,
  }) async {
    ...
    return snap.docs.map((d) => DiabetesHealthRecord.fromMap(d.data())).toList();
  }
}
